...
fs.readdirSync(normalizedPath).forEach(function(file) {
  localeData = Object.assign(
    localeData,
    YAML.load(path.join(normalizedPath, file))
  );
});
...
function handleRoute(req, res, routeConfig, matches) {
  ...
  if (routeConfig.query) {
    debugGraphql(params);
    return request(process.env.API_URL, routeConfig.query, params).then(
      data => {
        debugGraphql(data);
        data.params = params;
        data.meta = meta;
...
module.exports.router = function(req, res, next) {
  ...
  for (var name in routes) {
    ...
    for (var index in paths) {
      ...
      if (matches) {
        return handleRoute(req, res, routeConfig, matches).catch(err =>
          next(err)
        );
      }
    }
